...
We can look at reciever operating characteristics (ROC) with a ROC curve:
```{r}
library(ggplot2)
temp <- AUC::roc(output_probabilities$rf$PredictedProbs.1, labels = output_probabilities$rf$Truth)
...
rofi_results <- rofi(single_source_peppuRobj, source_alg_pairs, nn = 1, f_prob=0.4 ,
                     nu=1/100, conv_check = 30,maxIter=50,epsilon = 0.01,verbose=FALSE)
```
```{r}
rown <- length(rofi_results[[1]]$AllAUC)
fveci <- rofi_results[[1]]$f_Vectors[rown,]
final_fvec <- 0 +fveci/1
qplot(1:length(final_fvec),final_fvec)+xlab("Variable Index")+ylab("% Times Chosen")
```
