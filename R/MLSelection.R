#' Applies the MLWrapper to all data sources with all algorithms
#' 
#' This function returns performance and timing metrics for each source/algorithm combination
#' importFrom magrittr "%>%"
...
MLSelection <- function(data_object, partition_style = 'random', folds = 3, repeats = 10, algorithms = 'all', data_sources = 'all', single_source = NULL) {
  ...
  if(!is.null(attr(data_obj, "Partition_info"))){
    existing_partitions <- attr(data_obj, "Partition_info")
    attr(data_obj, "Partition_info") <- NULL
  }
