...
MLwrapper_helper = function(method, X, data, partition_info, scale_and_center, outcome_cname, pair_cname, sample_cname) {
    ...
    if (tolower(method) == "knn") {
        pred_label_knn = mapply(function(train, test) {
        ...
        pred_label_knn = as.data.frame(pred_label_knn)
        pred_label_knn = lapply(pred_label_knn, as.data.frame, stringsAsFactors = FALSE)
        result = pred_label_knn
