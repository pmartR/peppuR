...
.plot.mlSelect <- function(results_object, roc_curves = TRUE, time_chart = TRUE, x_lab = NULL, y_lab = NULL, plot_title = NULL, title_size = 14, x_lab_size = 11, y_lab_size = 11, bw_theme = FALSE) {
  ...
  if(roc_curves){
    plots <- lapply(seq_along(results_object), function(i,dsource) {
  ...
  if(time_chart){
    plot_data <- lapply(seq_along(results_object), function(i,dsource) {
